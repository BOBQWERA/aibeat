...
class Test5Cell(BaseCell):
    def __init__(self, x, y, cell_id,area_id, size) -> None:
        super().__init__(x, y, cell_id,area_id, size)
        self.cache = []

    def near_one_block(self,x,y,area,flag):
        if area[y][x] != BACKGROUND:
            return False
        count = 0
        if x>0 and area[y][x-1] == flag:
    ...
    def update(self, feelarea):
        ...
        area = list(feelarea)
        if self.energy>0:
            if cx>0 and self.near_one_block(cx-1,cy,area,self.area_id):
                return 'put','left'
            if cx<len(area[0])-1 and self.near_one_block(cx+1,cy,area,self.area_id):
                return 'put','right'
            if cy>0 and self.near_one_block(cx,cy-1,area,self.area_id):
                return 'put','up'
            if cy<len(area)-1 and self.near_one_block(cx,cy+1,area,self.area_id):
                return 'put','down'
        if self.energy>20 and random.random()>0.05:
            if cx>0 and area[cy][cx-1] == BACKGROUND:
                return 'put','left'
